...
class FirestoreServices implements DbBase {
  ...
  Future<bool> saveUser(UserModel userModel) async {
    ...
    DocumentSnapshot<Map<String, dynamic>> documentSnapshot = await firestore.doc("users/${userModel.userId}").get();
  ...
  Future<UserModel> readUser(String userId) async {
    DocumentSnapshot okunanUser = await firestore.collection("users").doc(userId).get();
  ...
  Future<bool> updateUserName(String userId, String newUserName) async {
    var users = await firestore.collection("users").where("userName",isEqualTo: newUserName).get();
    if (users.docs.length >= 1) {
      return false;
    } else {
      await firestore.collection("users").doc(userId) .update({"userName": newUserName});
      return true;
    }
  }

  Future<bool> updateProfilePhoto(String userId, String profilPhotoUrl) async {
    await firestore.collection("users").doc(userId).update({"profilUrl": profilPhotoUrl});
    return true;
  }
...
//sohbet ettiğim tüm kullanıcıları bir listeye aldım ve bu listeyi sohbetPage de göstereceğim.
  Future<List<KonusmaModel>> getAllConversations(String userId) async {
    QuerySnapshot querySnapshot = await firestore.collection("konusanlar").where("konusma_sahibi", isEqualTo: userId)
    .orderBy("olusturulma_tarihi", descending: true).get();
  ...
  Stream<List<MesajModel>> getMessages(String currentUserId, String sohbetEdilenUserId) {
    var snapshot = firestore.collection("konusanlar")
    .doc(currentUserId + "--" + sohbetEdilenUserId).collection("mesajlar").orderBy("date").snapshots();
    return snapshot.map(
      (snapshot) => snapshot.docs.map((event) => MesajModel.fromMap(event.data(),),).toList(),);
  }
  ...
  Future<DateTime> showTime(String userId) async {
    await firestore.collection("server").doc(userId).set({"saat": FieldValue.serverTimestamp()});
